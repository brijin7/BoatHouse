...
public partial class Reports_RptPrintAdditionalTicket : System.Web.UI.Page
{
    ...
    private void GetOtherTickets(string sBookingId)
    {
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = client.PostAsJsonAsync("PrintAdditionalTicket", BoatSearch).Result;
    ...
    public void BindAdditionalTicketDetails()
    {
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (UserRole == "Sadmin" || UserRole == "Admin")
                {
                    ...
                    response = client.PostAsJsonAsync("AdditionalTicketBookedDetailsV2", OtherBook).Result;
                }
                else
                {
                    var OtherBook = new OtherBook()
            ...
            using (var client = new HttpClient())
            {
                ...
                if (UserRole == "Sadmin" || UserRole == "Admin")
                {
                    ...
                    var OtherBook = new OtherBook()
                    {
                        ...
                        ToDate = txtToDate.Text.Trim(),
                        UserId = "Admin",
                        CountStart = txtSearch.Text
                    };
                    response = client.PostAsJsonAsync("AdditionalTicketBookedDetailsPinV2", OtherBook).Result;
                }
                else
                {
                    var OtherBook = new OtherBook()
        ...
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        Mpepnlrsn.Hide();
                        Response.Redirect("~/Boating/Print.aspx?rt=rat&bi=" + ViewState["PrBookingId"].ToString() + "");
