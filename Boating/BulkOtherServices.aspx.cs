...
public partial class Boating_BulkOtherServices : System.Web.UI.Page
{
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (StatusCode == 1)
                    {
                        ...
                        Response.Redirect("Print.aspx?rt=bo&bi=" + sResult[1].ToString() + "");
                    }
                    else
                    {
                        ClearBooking();
    ...
    protected void dtlOtherChild_ItemCommand(object source, DataListCommandEventArgs e)
    {
        ...
        if (chkKeyPad.Checked == true)
        {
            ...
            Panel pnlTicketKeypad = (Panel)((DataList)dtlOther).Controls[0].Controls[0].FindControl("pnlTicketKeypad");
            ...
            ScriptManager.RegisterStartupScript(Page, GetType(), "NumBox", "javascript: NumBox() ", true);
    ...
    protected void NumButton_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ViewState["TOServiceId"].ToString().Trim() == null || ViewState["TOServiceId"].ToString().Trim() == "")
            {
                txtNumberPass.Text = string.Empty;
                ScriptManager.RegisterStartupScript(Page, GetType(), "infoalert", "javascript: info(); ", true);
                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "NumBox();", true);
                return;

            }

            if (Convert.ToInt32(txtNumberPass.Text) > 200)
            {
                ScriptManager.RegisterStartupScript(Page, GetType(), "infoalertNumber", "javascript: infoalertNumber(); ", true);
                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "NumBox();", true);
                return;
            }

            string TicketsNumber = txtNumberPass.Text.Trim();
            ...
            BindDataDynamicValueOthers(ServiceId, ServiceName, ServiceTotalAmount, ChargePerItem, ChargePerItemTax, TicketsNumber, TaxId, TaxName);

            txtNumberPass.Text = string.Empty;
            ClearTempValues();
            //ScriptManager.RegisterStartupScript(Page, GetType(), "addItem", "javascript: addItem() ", true);
            ScriptManager.RegisterStartupScript(this, GetType(), "NumBox", "javascript: NumBox() ", true);
            Panel pnlTicketCount = (Panel)((DataList)dtlOther).Controls[0].FindControl("pnlTicketKeypad");
            pnlTicketCount.Visible = false;
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (StatusCode == 1)
                    {
                        ...
                        Mpepnlrsn.Hide();
                        Response.Redirect("Print.aspx?rt=bo&bi=" + ViewState["BoOkingID"].ToString().Trim() + "");
