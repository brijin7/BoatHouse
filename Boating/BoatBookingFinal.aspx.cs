...
public partial class Boating_BoatBookingFinal : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                divQrMob.Visible = false;
                if (Session["UserRole"].ToString().Trim() == "User")
                {
                    CreditBoatBooking.Visible = false;
                }
                else
                {
                    CreditBoatBooking.Visible = true;
                }
                if (Session["BMBooking"].ToString().Trim() == "Y")
                {
                    ...
                    {
                        BulkBoatBooking.Visible = true;
                    }
                    else
                    {
                        BulkBoatBooking.Visible = false;
                    }
    ...
    protected void dtlBoatchild_ItemCommand(object source, DataListCommandEventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    if (ViewState["SlotId"].ToString().Trim() != "0")
                    {
    ...
    protected void lblSlot_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < dt1.Rows.Count; i++)
        {
            ...
            //  string sStatuss = dt1.Rows[i]["Status"].ToString().Trim();
            ...
            BindChangeSlotGrid(BoatTypeName, BoatTypeid, BoatSeaterType, BoatSeaterid, BoatCount, BlockId,
                ViewState["SlotIdOld"].ToString(), ViewState["SlotIdDesc"].ToString(), Status);
            dvContent.Attributes.Add("style", "display:none;");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (StatusCode == 1)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            // 2023-01-30 Vediyappan.V add online payment block condition.
                            //ddlPaymentType.Items.Insert(ddlPaymentType.Items.Count, new ListItem("QRCode", "QR"));

                            if (Session["DeptOnlineRights"].ToString() == "Y")
                            {
                                ddlPaymentType.Items.Insert(ddlPaymentType.Items.Count, new ListItem("QRCode", "QR"));
                            }
                        }
    ...
    protected void btnBoatBooking_Click(object sender, EventArgs e)
    {
        string sQRSuccess = string.Empty;
        try
        {
            if (ddlPaymentType.SelectedValue.Trim() == "QR")
            {
                if (chkCustMobileNo.Checked == true)
                {

                }
                else if (txtQrMob.Text == "" || txtQrMob.Text == "Null")
                {
            ...
            if (ddlPaymentType.SelectedValue.Trim() == "QR")
            {
                if (chkCustMobileNo.Checked == true)
                {
                    ViewState["MobileNum"] = txtCustMobileNo.Text.Trim();
                }
                if (txtQrMob.Text == "" || txtQrMob.Text == "Null")
                {
                    if (chkCustMobileNo.Checked == true)
                    {
                ...
                if (chkCustMobileNo.Checked == false)
                {
                    ViewState["MobileNum"] = txtQrMob.Text.Trim();
                }
                if (bsTotal.InnerText.Trim() == "")
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "infoalert('Select Any One Boat !');", true);
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var BoatSearch = new BoatBookNew()
                    {
                        BookingDate = DateTime.Now.ToString("dd/MM/yyyy"),
                        BookingPin = txtPIN.Text.Trim(),

                        UserId = UserIds.ToString().Split('~')[0].Trim(),//1009~
                                                                         //MobileNo = txtCustMobileNo.Text.Trim() == "" ? "8248499568" : txtCustMobileNo.Text.Trim(),//txtCustMobileNo.Text.Trim()
                        MobileNo = ViewState["MobileNum"].ToString().Trim(),
                        EmailId = "",

                        PaymentMode = ddlPaymentType.SelectedItem.Text.Trim(),
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            string[] sResult = ResponseMsg.Split('~');
                            ViewState["TransactionNo"] = sResult[1].ToString().Trim();
                            hfBoatNature.Value = "N";
                            BoatBookedSummaryList();
                            ...
                            divBack.Style.Add("background-color", "White");
                            // string sSMSMbileNo = txtCustMobileNo.Text.Trim() == "" ? "8072771384" : txtCustMobileNo.Text.Trim();
                            string sSMSMbileNo = ViewState["MobileNum"].ToString().Trim();
                            SendSMS("DQRcode", sSMSMbileNo.ToString().Trim(), sResult[1].ToString().Trim(), btnBoatBooking.Text.Trim());
                            ViewState["QRAmount"] = btnBoatBooking.Text.Trim();
                            if (ViewState["ReferenceNoForQR"].ToString().Trim() != "")
                            {
                                string SMSMessage = GetSMSMessage(ViewState["ReferenceNoForQR"].ToString().Trim(), sSMSMbileNo.ToString().Trim());
                                if (SMSMessage.ToString().Trim() != "")
                                {
                                    imgQRCode.ImageUrl = GenerateQRCode(SMSMessage.ToString().Trim());
                                    sQRSuccess = "Success";
                                    if (sQRSuccess.Trim() == "Success")
                                    {
                                        BtnGenerateQRcode.Visible = true;
                                        // ScriptManager.RegisterStartupScript(this, GetType(), "Click", "hiddenButtonClick();", true);
                                    }
                                }
                            }
                            //ClearBooking();
    ...
    private void ClearBooking()
    {
        try
        {
            ...
            //ScriptManager.RegisterStartupScript(this, GetType(), "Click", "hiddenButtonClick();", false);
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "slideUpDownDiv();", true);
            dvContent.Attributes.Add("style", "display:none;");
    ...
    protected void btnOther_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            divBack.Style.Add("background-color", "#90f06d");
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "slideUpDownDiv();", true);
            dvContent.Attributes.Add("style", "display:none;");
        }
        catch (Exception ex)
            ...
            if (sMobileNo.Trim() != "" && sMobileNo.Trim().Length == 10)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (StatusCode == 1)
                        {
                            ViewState["ReferenceNoForQR"] = ResponseMsg.ToString().Trim();
                        }
                        else
                        {
                            ViewState["ReferenceNoForQR"] = "";
                            //ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "infoalert('" + ResponseMsg.ToString() + "');", true);
                        }
                    }
                    else
    ...
    protected void ChkBulk_CheckedChanged(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("BulkBoatBooking.aspx", true);

        }
        else
        {
            Response.Redirect("BulkBoatBooking.aspx", false);
    ...
    protected void ChkCredit_CheckedChanged(object sender, EventArgs e)
    {
        if (ChkBulk.Checked == true)
        {
            Response.Redirect("CreditBoatBooking.aspx", true);

        }
        else
        {
            Response.Redirect("CreditBoatBooking.aspx", false);
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (dtExists.Rows.Count > 0)
                    {
                        ...
                        if (dtExists.Rows[0]["Individual"].ToString() == "" || dtExists.Rows[0]["Individual"].ToString() == "0")
                        {
    ...
    private void OtherBookingFinalForQR()
    {
        ViewState["OthServiceStatus"] = "";
        ViewState["OthServiceId"] = "";
        ViewState["OthChargePerItem"] = "";
        ...
        try
        {
            ...
            ViewState["OthServiceStatusNew"] = ViewState["OthServiceStatus"].ToString().TrimEnd('~');
            ViewState["OthServiceIdNew"] = ViewState["OthServiceId"].ToString().TrimEnd('~');
            ViewState["OthChargePerItemNew"] = ViewState["OthChargePerItem"].ToString().TrimEnd('~');
            ViewState["OthNoOfItemsNew"] = ViewState["OthNoOfItems"].ToString().TrimEnd('~');
            ViewState["sOthGSTNew"] = ViewState["sOthGST"].ToString().TrimEnd('~');
            ViewState["CGSTOthTaxAmountNew"] = ViewState["CGSTOthTaxAmount"].ToString().TrimEnd('~');
            ViewState["SGSTOthTaxAmountNew"] = ViewState["SGSTOthTaxAmount"].ToString().TrimEnd('~');
            ViewState["OthTaxDetailsNew"] = ViewState["OthTaxDetails"].ToString().TrimEnd('~');
            ViewState["OthNetAmountNew"] = ViewState["OthNetAmount"].ToString().TrimEnd('~');
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "erroralert('" + ex.Message.Replace("'", "").Replace("\r", "").Replace("\n", "") + "');", true);
        ...
        try
        {
            ...
            Count = ViewState["Count"].ToString();
            sCount = Count.Split('~');
            string FinalCount = ViewState["Count"].ToString().TrimEnd('~');
            string[] arrFinalCount = FinalCount.Split('~');
            string othNoOfItem = ViewState["OthNoOfItemsNew"].ToString();
            string[] arrothNoOfItem = othNoOfItem.Split('~');
            Available = ViewState["OthServiceStatus"].ToString();
            savailable = Available.Split('~');

            if (savailable[0] == "Y")
            {
                if (arrothNoOfItem.Count() == arrFinalCount.Count())
                {
                }
                ...
                if (arrothNoOfItem.Count() > arrFinalCount.Count())
                {
                    ...
                    for (int i = arrFinalCount.Count(); i < arrothNoOfItem.Count(); i++)
                    {
                        string Counts;
                        string[] sCounts;
                        Counts = ViewState["Count"].ToString();
                        ...
                        string sGST = "0";
                        ViewState["sGST"] += sGST.ToString().Trim() + '~';
                        string CGSTTaxAmount = "0";
                        ViewState["CGSTTaxAmount"] += CGSTTaxAmount.ToString() + '~';
                        string SGSTTaxAmount = "0";
                        ViewState["SGSTTaxAmount"] += SGSTTaxAmount.ToString() + '~';
                        //string sGSTAmount = "";
                        //ViewState["sGSTAmount"] += sGSTAmount.ToString().Trim() + '~';
                        string BoatDeposits = "0";
                        ViewState["BoatDeposits"] += BoatDeposits.ToString().Trim() + '~';
                        string BoatTypeIds = "0";
                        ViewState["BoatTypeIds"] += BoatTypeIds.ToString().Trim() + '~';
                        string BoatMinTimes = "0";
                        ViewState["BoatMinTimes"] += BoatMinTimes.ToString().Trim() + '~';
                        string BoatSeaterIds = "0";
                        ViewState["BoatSeaterIds"] += BoatSeaterIds.ToString().Trim() + '~';
                        string InitBoatCharges = "0";
                        ViewState["InitBoatCharges"] += InitBoatCharges.ToString().Trim() + '~';
                        string RowerMinCharge = "0";
                        ViewState["RowerMinCharge"] += RowerMinCharge.ToString().Trim() + '~';
                        string boatOfferAmount = "0";
                        ViewState["boatOfferAmount"] += boatOfferAmount.ToString().Trim() + '~';
                        string TaxAmountDetlBoat = "0";
                        ViewState["TaxAmountDetlBoat"] += TaxAmountDetlBoat.ToString().Trim() + '~';
                        string InitNetAmounts = "0";
                        ViewState["InitNetAmounts"] += InitNetAmounts.ToString().Trim() + '~';
                ...
                {
                    for (int i = arrothNoOfItem.Count(); i < arrFinalCount.Count(); i++)
                    {
                        string Status = "N";
                        ViewState["OthServiceStatus"] += Status.ToString().Trim() + '~';
    ...
    public string GenerateQRCode(string SMSMessage)
    {
        ...
        try
        {
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                var QRd = new BoatBookNew()
                {
                    String1 = SMSMessage.ToString().Trim()
                };
    ...
    protected void btnGenerateQRTicket_Click(object sender, EventArgs e)
    {
        Boolean available = FindTicketStatus();
        if (available == true)
        {
            MpepnlrsnQR.Hide();
        ...
        else
        {
            ...
            lblStartResponse.Text = "Payment Failure of ₹" + ViewState["QRAmount"].ToString();
        }
    }
    public string GetSMSMessage(string ReferenceNo, string MobileNo)
    {
        try
        {
            using (var client = new HttpClient())
            {
                ...
                var GetSMSMessage = new BoatBookNew()
                {
                ...
                HttpResponseMessage response = client.PostAsJsonAsync("GetSMSMessage", GetSMSMessage).Result;
    ...
    /// <summary>
    /// Developed By SILAMBARASU D ,01 OCT 2021
    /// </summary>
    /// <param name="TransactionNo"></param>
    /// <returns></returns>
    private Boolean FindTicketStatus()
    {
        try
        {
            using (var client = new HttpClient())
            {
                ...
                var GenrateTicketaftn = new GenerateTicket()
                {
                    ...
                    ServiceType = ViewState["TransactionNo"].ToString(),
                    BookingDate = ""
                };
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (dtExists.Rows.Count > 0)
                    {
                        ...
                        {
                            if (dtExists.Rows[i]["BookingId"].ToString() != "")
                            {
                                string BookingId = dtExists.Rows[i]["BookingId"].ToString();
                                Response.Redirect("PrintBoat.aspx?rt=b&bi=" + BookingId.ToString() + "");
                                return true;
                            }
                        }
                        return true;
    ...
    protected void ddlPaymentType_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlPaymentType.SelectedValue == "QR")
        {
            if (chkCustMobileNo.Checked == true)
            {
                divQrMob.Visible = true;
                divQrMobNoVisible.Visible = false;
                divQrStyle.Attributes.Add("style", "margin-left:121px;");
            }
            else
            {
                divQrMob.Visible = true;
                divQrMobNoVisible.Visible = true;
                divQrStyle.Attributes.Add("style", "margin-left:0px;");
            }
        }
    ...
    protected void btnCloseQRTicket_Click(object sender, EventArgs e)
    {
        MpepnlrsnQR.Hide();
        divBack.Style.Add("background-color", "white");
        ClearBooking();
