...
public partial class EmployeeUser : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Session["UserRole"].ToString().Trim() == "Admin" || Session["UserRole"].ToString().Trim().ToUpper() == "ADMIN"
               || Session["UserRole"].ToString().Trim() == "Sadmin" || Session["UserRole"].ToString().Trim().ToUpper() == "SADMIN")
            {
                int istart;
                int iend;
        ...
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var EmpMstr = new employeeMaster()
                {
                    ...
                    EmpTypeName = gvmstrEmployee.DataKeys[gvrow.RowIndex]["EmpTypeName"].ToString().Trim(),
    ...
    public void GetBranch()
    {
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = client.PostAsJsonAsync("CommonReport", BranchType).Result;
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (dtExists.Rows.Count > 0)
                    {
                        ...
                        ddlBranchCode.DataValueField = "BoatHouseId";
                        ddlBranchCode.DataTextField = "BoatHouseName";
                ...
                ddlBranchCode.Enabled = false;
                ddlBranchCode.SelectedValue = Session["BoatHouseId"].ToString().Trim();
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (dtExists.Rows.Count > 0)
                    {
                        ...
                        ddlRole.DataSource = dtExists.AsEnumerable()
                                                        .Where(x => x.Field<Int64>("ConfigId") != 1)
                                                        .CopyToDataTable();
    ...
    public void BindEmployee()
    {
        try
        {
            using (var client = new HttpClient())
            {
                ...
                var BranchType = new employeeMaster()
                {
                    QueryType = "GetEmpMasterV2",
                    ServiceType = "User",
                    CorpId = "",
                    Input1 = "3",
                    Input2 = Session["BoatHouseId"].ToString().Trim(),
                    Input3 = "",
                    Input4 = hfstartvalue.Value.Trim(),
                    Input5 = hfendvalue.Value.Trim(),
    ...
    //Bind Employee Single
    public void BindEmployeeSingle()
    {
        try
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    QueryType = "GetEmpMasterV2",
                    ServiceType = "UserSingle",
                    CorpId = "",
                    Input1 = "3",
                    Input2 = Session["BoatHouseId"].ToString().Trim(),
                    Input3 = txtSearch.Text,
                    Input4 = "",
                    Input5 = "",
