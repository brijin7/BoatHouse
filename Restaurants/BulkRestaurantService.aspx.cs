...
public partial class Restaurants_BulkRestaurantService : System.Web.UI.Page
{
		...
		try
		{
			if (!IsPostBack)
			{
				...
				if (Session["BMBookingRestaurant"].ToString().Trim() == "Y")
				{
					...
					ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "NumBox();", true);
        ...
        try
        {
            if (getList() == 1)
            {
                using (var client = new HttpClient())
                {
                    ...
                    {
                        Category = ViewState["CategoryId"].ToString(),
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (StatusCode == 1)
                    {
                        ...
                        {
                            ScriptManager.RegisterClientScriptBlock(Page,GetType(), "Popup", "successalert('e-Receipt Link Send to Customer Mobile !');",true);
                        ...
                        else
                        {
                            ...
                            //ClearBooking();
                            Response.Redirect("~/Boating/Print.aspx?rt=BulkRes&bi=" + sResult[1].ToString() + "");
                        }
    ...
    private void ClearBooking()
    {
        try
        {
            ...
            hfServiceId.Value = "";
            hfServiceName.Value = "";
            hfServiceTotalAmount.Value = "";
            hfChargePerItem.Value = "";
            hfChargePerItemTax.Value = "";
            hfTaxId.Value = "";
            hfTaxName.Value = "";
            ...
            chkKeyPad.Checked = true;
            pnlTicketKeypad.Visible = true;
            pnlTicketCount.Visible = false;
           
           
           
            if (ViewState["CustMobileNo"] == "C")
            {
                ClientScript.RegisterStartupScript(Page.GetType(), "NumBox", "javascript:NumBox();", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "NumBox", "javascript: NumBox() ", true);
            }
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (StatusCode == 1)
                    {
                        ...
                        Mpepnlrsn.Hide();
                        Response.Redirect("~/Boating/Print.aspx?rt=BulkRes&bi=" + ViewState["BoOkingID"].ToString().Trim() + "");
    ...
    protected void chkKeyPad_CheckedChanged(object sender, EventArgs e)
    {
        if (chkKeyPad.Checked == true)
        {
            pnlTicketKeypad.Visible = true;
            pnlTicketCount.Visible = false;
    ...
    protected void dlstTicketsCount_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (dlstTicketsCount.SelectedIndex == 0)
            {
                dlstTicketsCount.Focus();
                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "infoalert('Please, Select Any One Items !!!');", true);
                return;
            }

            if (hfServiceId.Value.Trim() == null || hfServiceId.Value.Trim() == "")
            {
                dlstTicketsCount.SelectedIndex = 0;
                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "infoalert('Please, Select Any One Items !!!');", true);
                return;
            }

            string TicketsCount = dlstTicketsCount.SelectedValue.Trim();

            string ServiceId = hfServiceId.Value.Trim();
            string ServiceName = hfCategoryName.Value.Trim() + " - " + hfServiceName.Value.Trim();
            string ServiceTotalAmount = hfServiceTotalAmount.Value.Trim();
            string ChargePerItem = hfChargePerItem.Value.Trim();
            string ChargePerItemTax = hfChargePerItemTax.Value.Trim();
            string TaxId = hfTaxId.Value.Trim();
            string TaxName = hfTaxName.Value.Trim();
            string AvailableQty = hfAvailableQty.Value.Trim();
            string Stock = hfStock.Value.Trim();
            ...
            else
            {
                ...
                BindDataDynamicValueOthers(ServiceId, ServiceName, ServiceTotalAmount, ChargePerItem, ChargePerItemTax, TicketsCount, TaxId, TaxName, AvailableQty, Stock);
            }
    ...
    {
        Response.Redirect("BookingRestaurantServices.aspx");
    }
    ...
    protected void btnGet_Click(object sender, EventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(Page, GetType(), "addItem", "javascript: addItem() ", true);
        ...
        {
            BindDataDynamicValueOthers(hfServiceId.Value, hfCategoryName.Value + " - " + hfServiceName.Value,
           hfServiceTotalAmount.Value, hfChargePerItem.Value, hfChargePerItemTax.Value, hfAdultCount.Value, hfTaxId.Value, hfTaxName.Value, hfAvailableQty.Value, hfStock.Value);
